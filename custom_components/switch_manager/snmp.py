...
class SwitchSnmpClient:
    ...
    async def _async_walk_ipv4(self) -> None:
        ...
        2) If legacy is empty, fill **only ipIndex** via modern IP-MIB (ipAddressIfIndex + ipAddressAddr).
           We intentionally do NOT compute mask bits here (user requested IP only).
        """
        # Local helpers (kept here to avoid touching other modules)
        ...
        ip_mask: Dict[str, str] = {}  # left untouched in the fallback path
        ...
        # ---- (2) Modern fallback: ipAddressIfIndex + ipAddressAddr -> ipIndex only ----
        ...
        # Store only what we found; leave ipMask alone
        if ip_index:
            self.cache["ipIndex"] = ip_index
